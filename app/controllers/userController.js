...
let signUpFunction = (req, res) => {
    ...
    validateUserInput(req, res)
        ...
        .then((resolve) => {
            ...
            fs.mkdir(`app/assets/uploads/${resolve.userId}`, function(err) {
                if (err) {
                  console.log(err)
                } else {
...
let loginFunction = (req, res) => {
    ...
    let validatePassword = (retrievedUserDetails) => {
        ...
        return new Promise((resolve, reject) => {
            retrievedUserDetails.validatePassword(req.body.password, (err, isMatch) => {
                if (err) {
                    console.log(err)
                    logger.error(err.message, 'userController: validatePassword()', 10)
                    let apiResponse = response.generate(true, 'Login Failed due to server error', 500, null)
                    reject(apiResponse)
                } else if (isMatch) {
...
let addToWatchList = (req, res) => {
    UserModel.updateOne(
        ...
        { $addToSet: { watchList: req.body.issueId} },
        function(err, result) {
          if (err) {
            logger.error(err.message, 'User Controller:addToWatchList', 10);

            let apiResponse = response.generate(true, 'Failed To create watch-list', 500, null);
            ...
            let apiResponse = response.generate(false, 'Watch-list details edited', 200, result);
...
let removeFromWatchList = (req, res) => {
    console.log('remove', req.body.userId)

    UserModel.updateOne(
        { userId: req.body.userId },
        { $pull: { watchList: req.body.issueId} },
        function(err, result) {
          if (err) {
            logger.error(err.message, 'Issue Controller:addToWatchList', 10);
